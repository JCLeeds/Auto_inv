...
def produce_final_GBISoutput(geoc_ml_path,output_geoc_ml_path, model, vertex_path, usgs_loc,seis_model ,argv=None):
...
def GBIS_GMT_OUTPUT_FORWARD_MODEL(ifgix):
    ...
    model_series = str(min_los) + '/' + str(max_los*1.25) +'/' + str((max_los - min_los)/100)

    max_res = np.max(resid[~np.isnan(resid)]) 
    min_res = np.min(resid[~np.isnan(resid)]) 
    res_series = str(min_res) + '/' + str(max_res*1.25) +'/' + str((max_res - min_res)/100)

    max_data = np.max(unw[~np.isnan(unw)].flatten()) 
    print(max_data)
    min_data = np.min(unw[~np.isnan(unw)].flatten()) 
    data_series = str(min_data) + '/' + str(max_data*1.25) +'/' + str((max_data - min_data)/100)
    print(data_series)

    max_los_usgs = np.max(los_grid_usgs) 
    min_los_usgs = np.min(los_grid_usgs) 
    model_series_usgs = str(min_los_usgs) + '/' + str(max_los_usgs*1.25) +'/' + str((max_los_usgs - min_los_usgs)/100)

    max_res_usgs = np.max(resid_usgs[~np.isnan(resid_usgs)]) 
    min_res_usgs = np.min(resid_usgs[~np.isnan(resid_usgs)]) 
    res_series_usgs = str(min_res_usgs) + '/' + str(max_res_usgs*1.25) +'/' + str((max_res_usgs - min_res_usgs)/100)
    ...
    with fig.subplot(
        ...
        for ii in range(0,3):
                fig.colorbar(frame=["x+lLOS displacment(m)", "y+lm"], position="JMB",projection='M?c',panel=[0,ii]) 
