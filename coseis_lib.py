...
def plot_data_model(x, y, U, model, data_unw, e2los, n2los, u2los, show_grid=False):
    ...
    if show_grid:
        ax[0].grid()
    ax[0].plot(np.array([c1x, c2x, c3x, c4x, c1x])/1000, np.array([c1y, c2y, c3y, c4y, c1y])/1000, color='black')
    cbar = fig.colorbar(im_u, ax=ax[0],fraction=0.046, pad=0.04)
    cbar.set_label('Displacement (mm)', rotation=270)
    ax[0].set_xlabel('Easting (km)')
    # ax[0].set_ylabel('Northing (km)')
    ax[0].set_title('Unwrapped interferogram (mm)')
    ...
    if show_grid:
        ax[1].grid()
    ax[1].plot(np.array([c1x, c2x, c3x, c4x, c1x])/1000, np.array([c1y, c2y, c3y, c4y, c1y])/1000, color='black')
    cbar = fig.colorbar(im_u, ax=ax[1],fraction=0.046, pad=0.04)
    cbar.set_label('Displacement (mm)', rotation=270)
...
def load_ifgs(example_name, downsamp=False):
    ...
    elif example_name == 'iran_turkey':
        unw_file = 'data/iran_turkey/iran_turkey.unw'
        diff_file = 'data/iran_turkey/iran_turkey.diff'
        param_file = 'data/iran_turkey/iran_turkey.par'
