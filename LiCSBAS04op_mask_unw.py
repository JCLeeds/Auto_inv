...
def main(argv=None,auto=None):
    ...
    global ifgdates2, in_dir, out_dir, length, width, bool_mask, cycle, cmap_wrap, cc_ifg_thre, circ_sig_mask
    ...
    if poly_file:
        ...
        # Removed for John Condon automation
        for poly_str in poly_strings_all:
            bool_mask = bool_mask + tools_lib.poly_mask(poly_str, lon, lat)
    
    # Circ mask added for automation by John Condon 
    if circ_sig_mask is not None: 
          
        print('Masking using circle around usgs') 
        dempar = os.path.join(in_dir, 'EQA.dem_par')
        lat1 = float(io_lib.get_param_par(dempar, 'corner_lat')) # north
        lon1 = float(io_lib.get_param_par(dempar, 'corner_lon')) # west
        postlat = float(io_lib.get_param_par(dempar, 'post_lat')) # negative
    ...
    if circ_sig_mask is None:
        ...
        ### Save image of mask
        mask = np.float32(~bool_mask)
        maskfile = os.path.join(out_dir, 'signal_mask')
        mask.tofile(maskfile)
