...
def read_ENU_to_phi_theta(path_to_date,length,width):
    """
    Takes array of file paths to [*geo.E.tif,*geo.N.tif,*geo.U.tif] and coverts using the same method outlined in 
    https://github.com/pyrocko/kite/blob/master/kite/scene_io.py scene_io.py LiCS returns array of Phi and Theta
    ...
    phi = np.arctan2(los_n,los_e)
    phi = -np.pi - phi
    theta = np.arccos(los_u)
    
    return np.degrees(phi), np.degrees(theta)

def stack_ifgms(path_to_dir,length,width):
    stacked_data = np.zeros((length,width))
    nan_checker = np.zeros((length,width))
    ii = 0
    for dir in os.listdir(path_to_dir):
        print(dir)
        if os.path.isdir(os.path.join(path_to_dir,dir)) and dir != 'semivariograms':
            print("IN THE LOOP")
            date = dir.split('/')[-1]
            data_path = os.path.join(os.path.join(path_to_dir,dir),(date + ".unw"))
            unw = read_img(data_path,length,width)
            stacked_data = stacked_data + unw 
            index = np.where(unw==np.nan,0,1)
            nan_checker = nan_checker + index 
            # nan_checker[]
            ii+=1
        else:
            pass 
    
    stacked_data = stacked_data/nan_checker
    print("STACKED DATA DIMENSIONS ==== " + str(np.shape(stacked_data))) 
    return stacked_data

def getLOS_tif( filename, component):
        path = os.dirname(filename)
        fn = glob.glob(os.join(path, component))
        if len(fn) != 1:
...
def getLOS_binary(ENU_path,length,width):
    los = np.fromfile(ENU_path,dtype=np.float32).reshape((length,width))
    print("size of los" + str(np.shape(los)))
    return los


def npz2mat(filepath):
    """
    Converts npz file to a .mat file added by J.C
    """
    from scipy.io import savemat
    npzFiles = glob.glob(filepath)
    for f in npzFiles:
